...
dz=10;
x=np.mgrid[-20500:20500+dxy:dxy]; # solve on a 5 km domain
...
stability_used=CONSTANT_STABILITY; #CONSTANT STABILITY, ANNUAL CYCLE

#PLAN VIEW, HEIGHT SLICE, SURFACE TIME
output=PLAN_VIEW;
x_slice=25; # position (1-50) to take the slice in the x-direction
y_slice=0;  # position (1-50) to plot concentrations vs time

#Contant, fluctuating, prevailing
wind=CONSTANT_WIND;
stacks=ONE_STACK;
stack_x=[0., 1000., -200.];
stack_y=[0., 250., -500.];
...
wind_speed=0.5*np.ones((days*24,1)); # m/s
if wind == CONSTANT_WIND:
   wind_dir=340.*np.ones((days*24,1));
...
if output == PLAN_VIEW:
   ...
   plt.pcolor(x,y,np.mean(C1,axis=2)*1e6, cmap='turbo');
   plt.clim((0, 1e2));
   ...
   plt.pcolor(y,z,np.mean(C1,axis=2)*1e6, cmap='turbo')      
   plt.clim((0,1e2));
   plt.xlabel('y (metres)');
   plt.ylabel('z (metres)');
   plt.title(stability_str + '\n' + wind_dir_str);
...
#     plt.ylabel('y (m)');
    ...
    cs.axes.get_xaxis().set_visible(False)
    cs.axes.get_yaxis().set_visible(False)
